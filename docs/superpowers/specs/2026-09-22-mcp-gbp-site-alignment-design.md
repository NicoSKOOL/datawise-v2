...
- No owner-side GBP data (services and products entered in the GBP editor, post
  insights, messaging). DataForSEO cannot read it. That is the future GBP API OAuth
  integration ([[project_gbp_integration_plan]]).
...
signals:       price_level, place_topics[] (topic + count), people_also_search[]
               ({title, rating, reviews_count, category, cid}), popular_times (detailed
               only)
raw_keys:      list of item keys not mapped above (so nothing is silently lost)
```

`handleGBPProfile` is changed to build `{ ...legacyFields, full: normalizeGbpProfile(item) }`
and cache that. Legacy fields are unchanged, so the SPA card and `datawise_gbp_audit`
keep working. `datawise_gbp_profile` reads `full`.
...
New DataForSEO call `/business_data/google/my_business_updates/live` with the same
keyword. Output per post: date, text, cta_type, cta_url, image_url. Plus `posts_count`,
`last_post_date`, `days_since_last_post`. Same null + error pattern on failure.
...
2. Homepage through DataForSEO instant pages (JavaScript on). Collect internal link
   URLs and anchors from the response. This also becomes page 1 of the result.
...
Instant pages call per URL (`load_resources: false`, `enable_javascript: true`,
`store_raw_html: true`), matching `fetchPageData` in `workers/src/chat/page-analyzer.ts`.
Up to 5 concurrent. From the response and raw HTML, a pure `extractPageFacts(html,
instantPagesItem)`:

```
url, status_code, blocked: boolean (bot-challenge detector), fetched_at
...
`estimateCostUsd`: `0.002 * (max_pages + urls.length)`, cap $0.05. Actual from
DataForSEO. The homepage counts as one page.

### 4.6 Runtime

Worst case 25 instant-pages calls at 5 concurrent, ~3 to 5s each, plus sitemap fetches:
about 20 to 30s wall time. Within the Worker's limits and the MCP clients' timeouts
(claude.ai tolerates 60s+). If DataForSEO exceeds 8s on a page, that page is dropped
and counted in `skipped_urls_count` rather than failing the call.
...
- Tool tests assert the exact DataForSEO request bodies for profile, reviews task_post,
  posts, instant pages (lesson from the `datawise_ai_mentions` bug: never assert against
  a guessed shape).
