...
- Active promo coupons must also bypass the Worker `checkAndDeductCredit` free credit gate, not just the frontend credit display. Promo users can still be `subscription_tier='free'` with `credits_used >= 5`, so always check `promo_redemptions.expires_at > datetime('now')` before blocking.
- Keyword Research result tables must preserve the Content Planner save workflow. Overview, Related, Suggestions, Ideas, Difficulty, People Also Ask, and Fan-out Queries rows should support selecting keywords/questions and saving them to `/content-planner`, not only CSV export.
- Keyword Research result table sorting must treat `competition_level` as a semantic bucket, not alphabetical text. Low-to-high order is `LOW`, `MEDIUM`, `HIGH`, then unknown/blank values; high-to-low should keep unknown/blank values at the end.
- The shared staging frontend alias is `https://preview-1.datawise-118.pages.dev`. Branch-specific Pages previews are useful for isolated checks, but when the user asks for the staging site, deploy and verify the `preview-1` alias.
- Content Planner saves are property-scoped. Manual/non-GSC sites must remain selectable even when `/gsc/properties` returns `connected: false`; otherwise users can select keyword rows but cannot save them because no `property_id` is available.
...
## Deploys + rollback paper trail
- Every production deploy needs a paper trail: tag the source commit `prod-<UTC-stamp>`, record the worker version id (from `wrangler deployments list`), and append both to DEPLOY.md's "Named recovery tags" + "Rollback (Worker)" sections with a one-line description of what changed. If a change ships without that paper trail, the next incident becomes a guessing game.
- Resend uses `www.datawiseseo.com` as its click-tracking domain. Every transactional email link is rewritten through `https://www.datawiseseo.com/CL0/<encoded-target>/...`, decoded by the Pages Function `marketing/functions/CL0/[[path]].ts`. Astro builds to `dist/` and Cloudflare Pages only picks up Functions inside the deploy directory, so the marketing `build` script MUST end with `cp -R functions dist/functions`. Without it, every reset/invite/welcome email link 404s silently.
- DataForSEO returns HTTP 402 when the daily account cap is hit (balance can still be positive — it's a per-day total limit on the DataForSEO dashboard, not an account-empty signal). When this happens, every uncached SERP/Labs subrequest still burns ~3.8s of Worker wall time before failing. The worker client at `workers/src/dataforseo/client.ts` now (a) throws a typed `DataForSeoQuotaError`, (b) writes a per-UTC-day KV flag so subsequent calls short-circuit instantly, and (c) the top-level catch maps it to HTTP 503 `provider_quota_exhausted` with a user-friendly message. Operational fix is to raise the daily cap in the DataForSEO dashboard.
