...
serve(async (req) => {
  ...
  try {
    ...
    const { targets, location_code = 2840, language_code = 'en' } = await req.json();
    
    if (!targets || !Array.isArray(targets) || targets.length === 0) {
      return new Response(JSON.stringify({ error: 'Targets array is required and must not be empty' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }

    console.log('Processing bulk traffic estimation for:', targets.length, 'targets');
    ...
    console.error('Error in bulk-traffic-estimation function:', error);
