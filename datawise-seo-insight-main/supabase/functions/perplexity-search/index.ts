...
serve(async (req) => {
  ...
  try {
    ...
    const { keyword, location_code = "US" } = await req.json();
    ...
    console.log(`Processing Perplexity Search for keyword: ${keyword}`);
    ...
    const requestBody = {
      user_prompt: keyword,
      model_name: "sonar",
      web_search_country_iso_code: location_code
    };

    console.log('Making request to DataForSEO Perplexity API...');

    const response = await fetch('https://api.dataforseo.com/v3/ai_optimization/perplexity/llm_responses/live', {
      method: 'POST',
      headers: {
    ...
    return new Response(
      JSON.stringify({ 
        error: 'Failed to fetch Perplexity Search data',
