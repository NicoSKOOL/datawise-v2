  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'signup' && /already exists/i.test(message)) {
        // Server returns 409 "An account with this email already exists".
        // Steer the user to the login path instead of a dead-end error.
            ...
            <>
              ...
              <form onSubmit={handleEmailSubmit} className="space-y-4">
                ...
                {isSignup && existingAccount && (
                  <div role="alert" className="rounded-lg border border-border bg-secondary/50 p-3 text-sm">
                    <p className="font-medium text-foreground">
                      This email already has an account.
                    </p>
                    <button
                      type="button"
                      onClick={() => { setPassword(''); setAuthMode('login'); }}
