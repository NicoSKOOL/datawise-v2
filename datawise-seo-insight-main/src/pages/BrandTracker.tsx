...
export default function BrandTracker() {
  ...
  // Trend chart runs on the Historical endpoint, not the mention list. DFS
  // returns exact monthly totals, so the curve no longer depends on how many
  // rows the list has loaded (bug 1175b102) and one call replaces a 1000-row
  // search. Platforms are requested separately because DFS merges them
  // otherwise, which would break the toggles.
  const googleChart = useQuery({
    queryKey: ['llm-historical', activeDomain, 'google'],
    queryFn: () =>
      fetchLlmHistorical({
        target: [{ domain: activeDomain, include_subdomains: true }],
        platform: 'google',
  ...
  const chatgptChart = useQuery({
    queryKey: ['llm-historical', activeDomain, 'chat_gpt'],
    queryFn: () =>
      fetchLlmHistorical({
        target: [{ domain: activeDomain, include_subdomains: true }],
        platform: 'chat_gpt',
  ...
  return (
    <div className="space-y-6">
      ...
      <div id="brand-tracker-trend-export">
        <MentionsTrendChart
          googleItems={googleActive ? googleChart.data?.items : undefined}
          chatgptItems={chatgptActive ? chatgptChart.data?.items : undefined}
