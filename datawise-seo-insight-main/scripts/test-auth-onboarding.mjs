...
const checks = [
  ...
  [
    'Duplicate-email signup shows an inline log-in nudge',
    /already exists/i.test(authPage)
      && /This email already has an account\./.test(authPage)
      && /Log in instead/.test(authPage)
      && /setExistingAccount/.test(authPage),
  ],
  [
    'Existing-account nudge routes back to the login path',
    /setPassword\(''\); setAuthMode\('login'\)/.test(authPage),
  ],
];
