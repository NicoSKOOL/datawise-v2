...
const RANKED_KEYWORDS_STORAGE_KEYS_SQL = `
  SELECT DISTINCT a.storage_key AS storage_key
  FROM evidence_refs e
  JOIN artifacts a ON a.id = e.artifact_id
...
async function distinctStorageKeys(d1: D1Database, runId: string, sql: string): Promise<string[]> {
  const { results } = await d1.prepare(sql).bind(runId).all<{ storage_key: string }>();
...
// a genuine D1 error (from distinctStorageKeys, not this function) is allowed
// to throw -- a single bad artifact must never sink the whole readback.
...
function extractKeywordEnrichmentItem(item: any): { keyword: string; enrichment: KeywordEnrichment } | null {
  ...
  const mainIntentRaw = searchIntentInfo?.main_intent;

  return {
    keyword,
    enrichment: {
      coreKeyword: typeof props?.core_keyword === 'string' ? props.core_keyword : null,
      mainIntent: typeof mainIntentRaw === 'string' ? (mainIntentRaw.toLowerCase() as SearchIntent) : null,
...
export async function loadKeywordEnrichmentFromArtifacts(
  ...
  const storageKeys = await distinctStorageKeys(d1, runId, ENRICHMENT_STORAGE_KEYS_SQL);
  const data = new Map<string, KeywordEnrichment>();
  let artifactsRead = 0;
  let artifactsMissing = 0;

  for (const storageKey of storageKeys) {
    const items = await fetchArtifactItems(r2, storageKey);
    if (items === null) {
      artifactsMissing += 1;
      continue;
    }
...
export async function loadCompetitorRankingUrls(
  ...
  const storageKeys = await distinctStorageKeys(d1, runId, RANKED_KEYWORDS_STORAGE_KEYS_SQL);
